...
removes the file. The private state stores bounded receipts that pair the
SHA-256 slot-path hash with the SHA-256 event hash. It scans both bounded inboxes
for presence before pruning receipts. A committed receipt therefore survives a
temporary read or deletion lock and a second queue that fills the 256-event
processing window. Temporarily unreadable files are deferred. DPAPI failure is
handled as one malformed entry inside the per-file boundary, so it cannot abort
later events or the enclosing cycle.
...
that event. A locked file remains receipted and cannot replay, even when the
other bounded inbox fills one processing window. Hook
