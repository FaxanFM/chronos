...
Assignments without a bounded objective, scope, permission, exclusions, or completion criteria must be rejected or returned for clarification. A write assignment uses one same-repository writer by default. Read-only workers may run concurrently when their work does not conflict.
...
The report is evidence, not acceptance. The coordinator must inspect the actual workspace, changed-file list, diff, tests, and generated artifacts. Reports and coordination state must contain no prompts, responses, secrets, source contents, or raw tool output.
...
2. Confirm the worker identity and effective model when required.
